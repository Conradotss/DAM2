...
import Clases.Telegram.Usuario.DatosUsuario;
...
//            System.out.println(id + nombre + tipoUsuario);
            //Buscamos en la base de datos
            ControladorUsuarios controlador = new ControladorUsuarios();
            ArrayList<Usuario> lista = controlador.buscarUsuario(telegram, nombre);

            String htmlcode = "<table class=\"tabla\" id=\"tablaUsuario\">\n"
            ...
            for (int i = 1; i < DatosUsuario.nombreTitulosCabeceraTabla.length; i++) {
                htmlcode += "<th>";
                htmlcode += DatosUsuario.nombreTitulosCabeceraTabla[i];
                htmlcode += "</th>";
            }
...
//                if (Permisos2.getEliminar() == 1) {
                htmlcode += "<button type=\"button\" class=\"buttonTablaEliminar\" id=\"btnEliminarUsuario\" value=" + lista.get(i).getBasedatosID() + " "
                        + "title=\"Eliminar " + lista.get(i).getNombre() + "\"></button>\n"
                        + "\n";
                //boton de modificar
//                }
//                if (Permisos2.getModificar() == 1) {
                htmlcode += "<button type=\"button\" class=\"buttonTablaModificar\" id=\"btnModificarUsuario\" value=" + lista.get(i).getBasedatosID() + " "
